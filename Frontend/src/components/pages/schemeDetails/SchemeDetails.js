...
//     // Helper: safely render values that may be string or object like {label: "..."}
//     const renderValue = (val) => {
//     if (val === null || val === undefined) return null;
//     if (typeof val === 'string' || typeof val === 'number' || React.isValidElement(val)) return val;
//     if (typeof val === 'object') {
//         // prefer .label or .name
//         if (val.label) return val.label;
//         if (val.name) return val.name;
//         // for arrays or other objects, try to join strings if possible
//         if (Array.isArray(val)) return val.map(v => (typeof v === 'object' ? (v.label || v.name || JSON.stringify(v)) : String(v))).join(', ');
//         // fallback to JSON string (useful for debugging)
//         try {
...
//         try {
//         const savedSchemes = await getFavoriteSchemes();
//         const isSaved = Array.isArray(savedSchemes) ? savedSchemes.includes(id) : false;
//         setIsSaved(isSaved);
//         } catch (error) {
//         console.error('Error checking saved status:', error);
//         }
...
//             {/* Header Section */}
//             <header className="border-b border-gray-100 pb-6">
//             <h1 className="text-4xl font-bold mb-3 text-gray-900">{renderValue(scheme?.schemeName)}</h1>
//             {scheme?.schemeShortTitle && (
//                 <p className="text-gray-500 text-lg">({renderValue(scheme.schemeShortTitle)})</p>
//             )}

//             {/* Tags */}
//             <div className="mt-4 flex flex-wrap gap-2">
//                 {scheme?.tags?.map((tag, index) => {
//                 const txt = renderValue(tag);
//                 return (
//                     <span
//                     key={index}
//                     className="px-4 py-1.5 bg-green-50 text-[#74B83E] rounded-full text-sm font-medium"
...
//             <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 rounded-xl p-4">
//                 {scheme?.nodalMinistryName && (
//                 <p className="text-gray-600">Ministry: <span className="font-medium text-gray-900">{renderValue(scheme.nodalMinistryName)}</span></p>
//                 )}
//                 {scheme?.state && (
//                 <p className="text-gray-600">State: <span className="font-medium text-gray-900">{renderValue(scheme.state)}</span></p>
//                 )}
//                 {scheme?.level && (
//                 <p className="text-gray-600">Level: <span className="font-medium text-gray-900">{renderValue(scheme.level)}</span></p>
//                 )}
//                 {scheme?.openDate && (
//                 <p className="text-gray-600">Open Date: <span className="font-medium text-gray-900">{formatDate(scheme.openDate)}</span></p>
//                 )}
...
//                 {scheme?.applicationProcess?.map((process, index) => (
//                     <div key={index} className="bg-gray-50 rounded-xl p-6">
//                     <h3 className="font-semibold text-gray-800 mb-3">{renderValue(process?.mode)}:</h3>
//                     <DisplayFormatted benefitsData={process?.process} />
//                     </div>
//                 ))}
//                 </div>
...
//                 {scheme.faqs.map((faq, index) => (
//                     <div key={index} className="bg-gray-50 rounded-xl p-6">
//                     <h3 className="font-semibold text-gray-900 mb-2">{renderValue(faq.question)}</h3>
//                     <p className="text-gray-600">{renderValue(faq.answer)}</p>
//                     </div>
//                 ))}
//                 </div>
//             )}
            ...
            <div className="max-w-4xl mx-auto p-6">
                ...
                <div ref={contentRef} className="bg-white rounded-2xl shadow-lg p-8 space-y-8">
                    ...
                    <div className="py-4">
                        ...
                        {activeTab === 'benefits' && (
                            <div className="bg-gray-50 rounded-xl p-6">
                                <DisplayFormatted contentData={scheme?.benefits} dataType="benefits" />
                            </div>
                        )}

                        {activeTab === 'documents' && (
                            <div className="bg-gray-50 rounded-xl p-6">
                                <DisplayFormatted contentData={scheme?.documents_required} dataType="documents" />
                            </div>
                        )}

                        {/* {activeTab === 'apply' && (
                            <div className="space-y-4">
                                <DisplayFormatted contentData={scheme?.applicationProcess} dataType="applicationProcess" />
                            </div>
                        )} */}
                        ...
                        <div className="space-y-4">
                            ...
                            {/* DEBUG: show raw JSON so you can verify what arrived from backend */}
                            {/* <div className="bg-gray-100 rounded p-4 mt-4 text-sm text-gray-700">
                            <strong>DEBUG: raw applicationProcess JSON:</strong>
                            <pre className="whitespace-pre-wrap mt-2 text-xs">
                                {JSON.stringify(scheme?.applicationProcess, null, 2)}
                            </pre>
                            </div> */}
                        </div>
                        )}
